...
# ============================
output_dir <- "../Final_lagged/Covariates/Without_largerZ"
...
run_complex_sim <- function(n = 1000, T_points = 4, seed = 101, a = 0, ncores = 70) {
  ...
  # Define true effect sizes and noise levels
  ...
  single_sim <- function(l) {
    b1 <- b1_values[l]
    se <- se_values[l]
    c <- 100 * b1  # Strong effect of Z on Y
  ...
  # ============================
  # Compute overall power and type I error (lagged model)
  # ============================
...
for (nn in n_values) {
  ...
  # Run the simulation for this sample size
  result <- run_complex_sim(n = nn, a = a)
  results_list[[as.character(nn)]] <- result

  # Create combined wide-format plots
