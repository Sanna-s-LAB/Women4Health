...
if (!dir.exists("../Final_lagged/Covariates/With_Z_simulation")) {
  dir.create("../Final_lagged/Covariates/With_Z_simulation")
}

# Set working directory
setwd("../Final_lagged/Covariates/With_Z_simulation")

# Main simulation function
run_complex_sim <- function(n = 1000, T_points = 4, seed = 101, a = 0, ncores = 70) {
  # set.seed(seed)  # Optional: uncomment for reproducibility

  # True effect sizes and noise levels to explore
  b1_values <- seq(-2, 2, length.out = 200)
  se_values <- rnorm(200, 2, 0.5)

  # Single simulation for one value of b1
  single_sim <- function(l) {
    ...
    # Convert to wide format (base R reshape)
    wide_data <- reshape(
      data,
      timevar = "t",
      idvar = "id",
    ...
    # Wide-format models at different time points
    fit_lm_YtoX_1 <- fit_model_adaptive(data = wide_data, X.2 ~ Y.1 + X.1 + Z.2, mode = "wide")
    fit_lm_XtoY_1 <- fit_model_adaptive(data = wide_data, Y.2 ~ X.1 + Y.1 + Z.2, mode = "wide")
    ...
    fit_lm_YtoX_2 <- fit_model_adaptive(data = wide_data, X.3 ~ Y.2 + X.2 + Z.3, mode = "wide")
    fit_lm_XtoY_2 <- fit_model_adaptive(data = wide_data, Y.3 ~ X.2 + Y.2 + Z.3, mode = "wide")
    ...
    fit_lm_YtoX_3 <- fit_model_adaptive(data = wide_data, X.4 ~ Y.3 + X.3 + Z.4, mode = "wide")
    fit_lm_XtoY_3 <- fit_model_adaptive(data = wide_data, Y.4 ~ X.3 + Y.3 + Z.4, mode = "wide")
  ...
  # Create plots and long-format data for a given model
  ...
  return(list(
    ...
    model1 = list(p_values = p_values_1, p_long = list_1$p_long,
                  plot_pvalue = list_1$plot_pvalue, plot_beta = list_1$plot_beta),
    model2 = list(p_values = p_values_2, p_long = list_2$p_long,
                  plot_pvalue = list_2$plot_pvalue, plot_beta = list_2$plot_beta),
    model3 = list(p_values = p_values_3, p_long = list_3$p_long,
                  plot_pvalue = list_3$plot_pvalue, plot_beta = list_3$plot_beta)
  ))
}
...
for (nn in n_values) {
  ...
  # Create and save wide-format combined plots
  ...
  # Extract long-format plots and data
  plots <- results_list[[as.character(nn)]][["model_lag"]]

  # Combine p-value, beta recovery, and performance plots
