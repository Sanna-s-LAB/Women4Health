...
# Read parameter a from command line arguments
a = as.numeric(commandArgs(trailingOnly = TRUE)[1])
cat("The a is", a, "\n")

# Create output directory if it does not exist
if (!dir.exists(paste0("../Final_lagged/Quadratic_simulation_with_", a))) {
  dir.create(paste0("../Final_lagged/Quadratic_simulation_with_", a))
}
setwd(paste0("../Final_lagged/Quadratic_simulation_with_", a))


# Run a full simulation and evaluate models
run_complex_sim <- function(n = 1000, T_points = 4, seed = 53, a = 0, ncores = detectCores() - 1) {
  # set.seed(seed)
  ...
  single_sim <- function(l) {
    ...
    # Define data-generating formulas
    f_list <- list(
      X ~ lag(X) + trend + error,
    ...
    # Wide-format models at different lags
  ...
  # Long-format summary for lag model
  ...
  # Compute decision categories (only for p-values)
...
for (nn in n_values) {
  ...
  result <- run_complex_sim(n = nn, a = a, seed = 101)
  ...
  # Save combined plot for long-format model
